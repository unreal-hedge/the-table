...
- **Deploys**: pushes to `main` auto-deploy the frontend (Vercel ↔ GitHub) **and** the game server (`.github/workflows/deploy-worker.yml` runs `party:deploy` whenever `party/**`, `shared/**`, `patches/**`, or the package files change; it gates on `test-engine`, `test-filter`, and `party:check` first). It needs the `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID` repo secrets. **Check the Actions tab after a server-touching push** — if the run is red or the secrets aren't set yet, fall back to `npm run party:deploy` by hand (needs `wrangler login`). The live server must never lag `main`.
- **Push at every commit**: never leave commits local. `git push` immediately after each commit lands. Local-only commits are a repo-rule violation — the whole point is the live site and the game server stay in lockstep with `main`.
